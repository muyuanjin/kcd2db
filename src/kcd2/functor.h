...
/*
   ...
   build a functor out of a function that is 'type supported' with the
   target functor - it need not have an exactly matching signature. By
   type supported I mean a function with the same number of arguments, of
   ...
   you've passed something supported to functor(). In addition you
   ...
   ftor = functor( fred );
   do5Times(ftor);
   //note ethel is not an exact match - ok, is supported
   ftor = functor( ethel );
   do5Times(ftor);
